...
trait RISCVCommon {
  ...
  def reg(x: Int) = (x & ((1 << 4) - 1)).U(5.W)
  def imm(x: Int) = (x & ((1 << 20) - 1)).U(21.W)

  /** This allows bits to be combined without being in a chisel Module
  *
    * @param l a sequence to be assembled into a single UInt
    * @return
    */
  def Cat(l: Seq[Bits]): UInt = (l.tail foldLeft l.head.asUInt){ case (x, y) =>
    assert(x.isLit() && y.isLit())
    ((x.litValue << y.getWidth) | y.litValue).U
  }
  ...
  def bimm(inst: UInt) = Cat(Cat(Seq.fill(20){inst_31(inst)}),
                             inst_7(inst), inst_30_25(inst), inst_11_8(inst), 0.U(1.W)).litValue()
  def uimm(inst: UInt) = Cat(inst_31(inst), inst_30_25(inst), inst_24_21(inst),
                             inst_20(inst), inst_19_12(inst), 0.U(12.W)).litValue()
  def jimm(inst: UInt) = Cat(Cat(Seq.fill(12){inst_31(inst)}), inst_19_12(inst),
                             inst_20(inst), inst_30_25(inst), inst_24_21(inst), 0.U(1.W)).litValue()
...
trait RandomInstructions extends RISCVCommon {
  import Instructions._
  ...
  def rand_inst = BigInt(rnd.nextInt(Int.MaxValue)).U
  def rand_addr = rnd.nextInt().U
  def rand_data       = BigInt(rnd.nextInt(Int.MaxValue))

  def instructions: List[UInt] = List(
