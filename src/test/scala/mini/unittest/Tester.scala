...
abstract class SimMem(word_width: Int = 4, depth: Int = 1 << 20, verbose: Boolean = true)(
  ...
  def loadMem(test: Seq[UInt]) {
    ...
    (0 until (test.size / chunk)).foreach { i =>
      val writeValue = (0 until chunk).foldLeft(BigInt(0)) { case (res, k) =>
        res | ((test(i * chunk + k) << 32.U) * (chunk - 1 - k).U)
      }
      write(i, writeValue)
    }
  }
...
class CoreMem(
  ...
  def process {
    ...
    if (!ireqQ.isEmpty) {
      ...
      if (verbose) logger println "MEM[%x] -> %s".format(ireq.addr, dasm(inst.U))
...
//class CoreTester(c: Core, args: MiniTestArgs) extends AdvTester(c, 16, args.logFile) with MiniTests {
//
...
class TileTester(c: Tile, args: MiniTestArgs) extends FreeSpec with ChiselScalatestTester {
  "testing a tile" in {
    test(c) { c =>
      c.io.nasti.ar.initSource()
      c.io.nasti.ar.setSourceClock(c.clock)

      lazy val arHandler = (ar: NastiReadAddressChannel) =>
        new TestNastiReadAddr(
          ar.id.peek().litValue.toInt, ar.addr.peek().litValue.toInt,
          ar.size.peek().litValue.toInt, ar.len.peek().litValue.toInt
        )

      fork {
        arHandler(c.io.nasti.ar.bits)
      }
    }
